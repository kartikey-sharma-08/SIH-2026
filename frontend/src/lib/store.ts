...
export function getActiveSource(): CustomSource {
  try {
    ...
    const saved = window.localStorage.getItem(ACTIVE_SOURCE_KEY);
...
export function setActiveSource(source: CustomSource) {
  try {
    ...
    window.localStorage.setItem(ACTIVE_SOURCE_KEY, JSON.stringify(dataToSave));
  } catch (e) {
...
export function getStoredRuns(): GeneratedRun[] {
  try {
    if (!hasLocalStorage()) return [];
    const saved = window.localStorage.getItem(GENERATED_RUNS_KEY);
    if (saved) return JSON.parse(saved);
  } catch (e) {
...
export function saveRun(run: GeneratedRun) {
  ...
  try {
    if (!hasLocalStorage()) return;
    window.localStorage.setItem(GENERATED_RUNS_KEY, JSON.stringify(runs));
  } catch (e) {
